(function () {
  ...
  const el = {
    ...
    animSource: document.getElementById('animSource'),
    animSpeed: document.getElementById('animSpeed'),
    btnAnimPlay: document.getElementById('btnAnimPlay'),
    btnAnimPause: document.getElementById('btnAnimPause'),
    btnAnimReset: document.getElementById('btnAnimReset'),
    animTimeLabel: document.getElementById('animTimeLabel'),
    animValueLabel: document.getElementById('animValueLabel'),
    animInfoLabel: document.getElementById('animInfoLabel'),
    animReadyBadge: document.getElementById('animReadyBadge'),
  ...
  function setAnimationEnabled(enabled) {
    [el.animSource, el.animSpeed, el.btnAnimPlay, el.btnAnimPause, el.btnAnimReset].forEach((node) => {
      node.disabled = !enabled;
    });
    el.animReadyBadge.textContent = enabled ? 'Siap diputar' : 'Belum siap';
    el.animReadyBadge.className = enabled ? 'badge ok-badge' : 'badge muted-badge';
  }
  ...
  function clearAnimation() {
    if (state.animator) {
      ...
      state.animator.setDatasets({}, 'obs');
    }
    setAnimationEnabled(false);
    el.animSource.value = 'obs';
    el.animSpeed.value = '6';
    el.animTimeLabel.textContent = 'Waktu: -';
    el.animValueLabel.textContent = 'Elevasi: -';
    el.animInfoLabel.textContent = 'Animasi belum siap. Jalankan Proses Data dulu.';
  }
  ...
  function loadDatasetIntoUI(dataset) {
    ...
    const sourceExtra = dataset.sourceUrl
      ? `<strong>Sumber</strong><span class="small-break"><a href="${dataset.sourceUrl}" target="_blank" rel="noopener noreferrer">${dataset.sourceUrl}</a></span>`
  ...
  function handleBuildIocUrl() {
    try {
      ...
      el.iocUrl.value = url;
      setStatus(`URL IOC berhasil dibuat.<br><a href="${url}" target="_blank" rel="noopener noreferrer"><code>${url}</code></a>`, 'ok');
    } catch (error) {
  ...
  async function handleFetchIoc() {
    ...
    setStatus(`Mengambil data dari IOC...<br><a href="${url}" target="_blank" rel="noopener noreferrer"><code>${url}</code></a>`, 'muted');
    ...
    try {
      ...
      setStatus(`Gagal mengambil data IOC: ${msg}`, 'err');
    }
  }

  function validateOptions(dataset, options) {
    if (!options.sensorKey) throw new Error('Sensor belum dipilih.');
    if (!(options.start instanceof Date) || isNaN(options.start.getTime())) throw new Error('Start date belum valid. Gunakan format YYYY-MM-DD HH:mm.');
    if (!(options.end instanceof Date) || isNaN(options.end.getTime())) throw new Error('End date belum valid. Gunakan format YYYY-MM-DD HH:mm.');
  ...
  function buildAnimationFrames(times, values, maxFrames = 720) {
    const clean = [];
    for (let i = 0; i < times.length; i += 1) {
      if (times[i] instanceof Date && !Number.isNaN(times[i].getTime()) && Number.isFinite(values[i])) {
        clean.push({ time: times[i], value: values[i] });
      }
    }

    if (!clean.length) {
      return { times: [], values: [], totalRaw: 0, totalFrames: 0, step: 1 };
    }

    const step = Math.max(1, Math.ceil(clean.length / maxFrames));
    const sampled = [];
    ...
    return {
      ...
      totalRaw: clean.length,
      totalFrames: sampled.length,
      step,
    };
  }

  function updateAnimationSection(analysis) {
    const observedFrames = buildAnimationFrames(analysis.sampledTimes, analysis.observed);
    const modelFrames = buildAnimationFrames(analysis.sampledTimes, analysis.model);

    state.animator.setDatasets({
      obs: observedFrames,
      model: modelFrames,
    }, el.animSource.value || 'obs');
    state.animator.setSpeed(el.animSpeed.value);
    setAnimationEnabled(true);

    const sourceLabel = el.animSource.value === 'model' ? 'Model' : 'Observasi';
    const currentMeta = sourceLabel === 'Model' ? modelFrames : observedFrames;
    el.animInfoLabel.textContent = `${sourceLabel} | ${currentMeta.totalFrames} frame animasi dari ${currentMeta.totalRaw} titik analisis`;
  }

  function handleAnimationSourceChange() {
    if (!state.animator) return;
    state.animator.useDataset(el.animSource.value);

    if (state.lastAnalysis) {
      const analysis = state.lastAnalysis.analysis;
      const meta = el.animSource.value === 'model'
        ? buildAnimationFrames(analysis.sampledTimes, analysis.model)
        : buildAnimationFrames(analysis.sampledTimes, analysis.observed);
      el.animInfoLabel.textContent = `${el.animSource.value === 'model' ? 'Model' : 'Observasi'} | ${meta.totalFrames} frame animasi dari ${meta.totalRaw} titik analisis`;
    }
  }
  ...
  function handleProcess() {
    ...
    try {
      ...
      updateAnimationSection(analysis);
  ...
  function initAnimator() {
    state.animator = new WaveAnimator(el.waveCanvas, el.animTimeLabel, el.animValueLabel, el.animInfoLabel);
    clearAnimation();
  }
  ...
  function init() {
    ...
    el.animSource.addEventListener('change', handleAnimationSourceChange);
    el.animSpeed.addEventListener('input', () => {
      if (state.animator) state.animator.setSpeed(el.animSpeed.value);
    });
    el.btnAnimPlay.addEventListener('click', () => state.animator && state.animator.play());
    el.btnAnimPause.addEventListener('click', () => state.animator && state.animator.pause());
    el.btnAnimReset.addEventListener('click', () => state.animator && state.animator.reset());
  }
